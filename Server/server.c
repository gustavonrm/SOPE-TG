...
int main (int argc, char *argv[]) {
  ...
  while (1) {
    ...
    int tmpFifo;
    char usrFifo[USER_FIFO_PATH_LEN];
    char usrPid[WIDTH_ID];
    strcpy (usrFifo, USER_FIFO_PATH_PREFIX);
    sprintf (usrPid,"%d",request.value.header.pid);
    strcat (usrFifo, usrPid);
  
    if ((read (srvFifo, &request, sizeof (request))) != 0)
      exit(FIFO_READ_ERR);
  
    //process user fifo name
    tmpFifo = open(usrFifo, O_WRONLY);
    if (tmpFifo != 0)
      exit(FIFO_OPEN_ERR);
